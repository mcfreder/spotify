...
export default function Menu() {
  ...
  const { status } = useDropdown(element)
  const service = useContext(GlobalContext)
  const [state, send] = useActor(service.auth)
  const { user } = state.context

  return (
    <React.Fragment>
      <button className={cx('btn')} ref={element}>
        ...
        <div>
          <svg height="16" width="16" viewBox="0 0 16 16">
            {status ? <path fill='currentColor' d="M13 10L8 4.206 3 10z"></path> : <path fill='currentColor' d="M3 6l5 5.794L13 6z"></path>}
          </svg>
        </div>
            ...
            <li>
              <button>
                ...
                <svg role="img" height="16" width="16" viewBox="0 0 16 16">
                  <path fill='currentColor' d="M15 7V1H9v1h4.29L7.11 8.18l.71.71L14 2.71V7h1zM1 15h12V9h-1v5H2V4h5V3H1v12z"></path>
                </svg>
              </button>
            </li>
            <li>
...
/**
 * Dropdown hook, 
 * note that this is reusable for any type of dropdown component.
 */
  ...
  function handleClick(event) {
    if (ref.current) {
      if (ref.current && !ref.current.contains(event.target)) {
        ...
        if (ref.current.contains(event.target) && status) {
          setStatus(false)
        } else {
          setStatus(true)
        }
